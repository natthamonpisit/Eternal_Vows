...
export const Gallery: React.FC = () => {
  ...
  /* 
    ========================================================================================
    🧠 CORE LOGIC: SMART MASONRY ALGORITHM (Shortest Column First)
    ========================================================================================
    [Note for Maintenance - บันทึกกันลืม]
    อัลกอริทึมนี้ทำหน้าที่ "เกลี่ยรูป" ลงในแต่ละเสาให้มีความสูงใกล้เคียงกันที่สุด
    (แก้ปัญหาเดิมที่ใช้ Grid หรือ Flex ธรรมดาแล้วท้ายตารางแหว่งไม่สวย)
    
    [How it works - หลักการทำงาน]
    Concept: "เทน้ำลงแก้วที่พร่องที่สุด" (Greedy Algorithm)
    
    1. สร้างถัง (Columns) ตามจำนวนที่ต้องการ 
       - Mobile ใช้ 2 เสา
       - Desktop ใช้ 3 เสา
    
    2. วนลูปรูปภาพทีละใบ (Iterate Images)
       - คำนวณ "น้ำหนัก" หรือ Aspect Ratio ของรูป (Height / Width)
       - รูปแนวตั้ง จะมีค่าเยอะ (กินพื้นที่แนวตั้งเยอะ)
       - รูปแนวนอน จะมีค่าน้อย
       
    3. เช็คความสูงเสา (Check Column Heights)
       - ก่อนจะวางรูปถัดไป ให้หาว่า "เสาไหนเตี้ยที่สุด" ณ วินาทีนั้น
       
    4. วางรูป (Place Image)
       - หย่อนรูปลงในเสาที่เตี้ยที่สุดทันที
       - บวกความสูงของรูปนั้นเพิ่มเข้าไปในความสูงรวมของเสานั้น
       
    Result: ไม่ว่ารูปจะมาแนวตั้งรัวๆ หรือสลับกัน ระบบจะ Auto-balance ให้ท้ายตารางเสมอกันมากที่สุด
  */
  const distributeToColumns = (items: GalleryItem[], colCount: number) => {
    // 1. Init Buckets (เตรียมเสาว่างๆ)
    const columns: GalleryItem[][] = Array.from({ length: colCount }, () => []);
    
    // 2. Track Heights (ตัวแปรเก็บความสูงสะสมของแต่ละเสา เริ่มต้นที่ 0)
    const colHeights = new Array(colCount).fill(0);

    items.forEach((img) => {
      // Calculate Ratio (Height / Width) ใช้เป็นหน่วยวัดความสูง
      // ถ้าไม่มีข้อมูลขนาด (เช่น Mock data) ให้สมมติว่าเป็นสี่เหลี่ยมจัตุรัส (Ratio = 1)
      const ratio = (img.height && img.width) ? (img.height / img.width) : 1;

      // 3. Find Shortest Column (หาเสาที่เตี้ยที่สุด)
      const minHeight = Math.min(...colHeights);
      const shortestColIndex = colHeights.indexOf(minHeight);

      // 4. Push Image (วางรูปลงเสานั้น)
      columns[shortestColIndex].push(img);
      
      // Update Height (บวกความสูงเพิ่ม)
        ...
        {loading ? (
          ...
          <FadeInUp delay="200ms">
             ...
             {/* 
                ...
                Using 'Shortest Column First' logic
             */}
