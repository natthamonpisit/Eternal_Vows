        ...
        <FadeInUp delay="100ms">
          <div className="bg-[#FAF9F6] p-8 md:p-12 rounded-xl border border-gold/20 shadow-lg max-w-2xl mx-auto">
             <div className="flex flex-col gap-6 items-center">
                ...
                {/* Option 2: Write a Wish (Primary Action) - Pink Taupe -> Cream Beige */}
                <button 
                  onClick={() => setIsWriting(true)}
                  className="group relative w-full max-w-sm flex items-center justify-center gap-4 px-6 py-5 bg-[#C08E86] text-white rounded-lg shadow-lg hover:bg-[#E6DABF] hover:text-charcoal transition-all duration-300 transform hover:-translate-y-1"
                >
                   <div className="w-10 h-10 rounded-full bg-white/20 text-white flex items-center justify-center group-hover:bg-charcoal/10 group-hover:text-charcoal transition-colors">
                   ...
                   <div className="text-left flex-1">
                      ...
                      <span className="block font-sans text-[10px] text-white/70 uppercase tracking-wider group-hover:text-charcoal/60 transition-colors">Sign the guestbook</span>
                   </div>
                </button>
...
const GuestbookForm: React.FC<{ onSuccess: () => void }> = ({ onSuccess }) => {
  ...
  if (status === 'success') {
    return (
      <div className="max-w-lg mx-auto bg-white p-12 shadow-xl rounded-lg text-center animate-fade-in border-t-4 border-[#C08E86]">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input 
            type="text" 
            required
            className="w-full bg-white text-charcoal border-b-2 border-gray-200 py-2 px-3 focus:border-[#C08E86] focus:outline-none font-sans text-lg font-medium transition-colors placeholder-gray-300 rounded-none"
        ...
        <div>
          ...
          <textarea 
            required
            className="w-full bg-white text-charcoal border-2 border-gray-100 rounded-md p-3 focus:border-[#C08E86] focus:outline-none font-sans text-base font-medium transition-colors placeholder-gray-300"
            rows={4}
        ...
        <div>
          ...
          {!previewUrl ? (
            <div 
              className="border-2 border-dashed border-gray-300 bg-gray-50 rounded-md p-6 text-center cursor-pointer hover:border-[#C08E86] hover:bg-white transition-all group"
              onClick={() => fileInputRef.current?.click()}
            >
              <div className="w-8 h-8 mx-auto mb-2 text-gray-400 group-hover:text-[#C08E86] transition-colors">
              ...
              <p className="text-gray-500 text-sm font-sans font-medium group-hover:text-[#C08E86]">Add a selfie or memory</p>
            </div>
          ) : (
        ...
        <button 
          ...
          className="w-full bg-[#C08E86] text-white py-4 px-6 font-sans font-bold uppercase tracking-widest hover:bg-[#E6DABF] hover:text-charcoal transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg mt-4 flex items-center justify-center gap-2 rounded-md"
        >
