          ...
          {!isWriting ? (
            ...
            <div className="animate-fade-in max-w-xl mx-auto">
                 <div className="flex flex-col gap-5 items-center">
                    ...
                    {/* REDUCED WIDTH & HEIGHT: max-w-xs, py-3 */}
                    <button 
                      onClick={() => window.open('/#live', '_blank')}
                      className="group relative w-full max-w-xs flex items-center gap-4 px-5 py-3 bg-[#FAF9F6] border border-gold/20 rounded-sm hover:border-gold hover:bg-white hover:shadow-lg transition-all duration-300"
                    >
                       {/* Icon Size: w-12 -> w-10 */}
                       <div className="w-10 h-10 rounded-full bg-white border border-gold/20 text-gold flex items-center justify-center group-hover:bg-gold group-hover:text-white transition-colors shrink-0">
                       ...
                       <div className="text-left flex-1">
                          {/* 
                              ...
                              Subtitle: text-[7px]
                          */}
                          <span className="block font-sans text-xs md:text-lg text-charcoal group-hover:text-gold transition-colors font-bold uppercase tracking-wide">View Live Wall</span>
                          <span className="block font-sans text-[7px] md:text-[9px] text-gray-400 uppercase tracking-wider mt-0.5">See all wishes on projector mode</span>
                       </div>
                       <svg className="w-4 h-4 text-gray-300 group-hover:text-gold transform group-hover:translate-x-1 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                    </button>

                    <div className="flex items-center gap-4 w-full max-w-xs px-4 opacity-40">
                       <div className="h-px bg-gold/50 flex-1"></div>
                       <span className="font-serif italic text-gold text-base">or</span>
                       <div className="h-px bg-gold/50 flex-1"></div>
                    </div>
                    ...
                    <button 
                      ...
                      className="group relative w-full max-w-xs flex items-center gap-4 px-5 py-3 bg-gold text-white rounded-full shadow-md hover:bg-[#E6DABF] hover:text-charcoal transition-all duration-300 transform hover:-translate-y-0.5"
                    >
                       {/* Icon Size: w-12 -> w-10 */}
                       <div className="w-10 h-10 rounded-full bg-white/20 text-white flex items-center justify-center group-hover:bg-white/40 group-hover:text-charcoal transition-colors shrink-0">
                       ...
                       <div className="text-left flex-1">
                          {/* 
                              ...
                              Subtitle: text-[8px]
                          */}
                          <span className="block font-sans text-sm md:text-lg font-bold uppercase tracking-wide">Leave a Wish</span>
                          <span className="block font-sans text-[8px] md:text-[9px] opacity-70 uppercase tracking-wider mt-0.5">Sign the guestbook</span>
                       </div>
                    </button>
